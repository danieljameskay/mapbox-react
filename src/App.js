...
class App extends React.Component {
    ...
    componentDidMount() {
      ...
      socket.onopen = () => {
        console.log("CLIENT: Websocket opened to server.");
        ...
        map.on('load', () => {
          ...
          map.addSource('driver', {
            ...
            setInterval(() => {
              socket.send(JSON.stringify({driverId: this.state.driverId, driverCoordinates: `${map.getSource('driver')._data.coordinates[1]},${map.getSource('driver')._data.coordinates[0]}`, dateTime: Date.now().toString()}));
            }, 2000);
          });
    ...
    return (
      <div>
        <div className="inline-block absolute top left mt12 ml12 bg-darken75 color-white z1 py6 px12 round-full txt-s txt-bold">
          <div>{`Longitude: ${lng} Latitude: ${lat} Zoom: ${zoom} DriverID: ${this.state.driverId}`}</div>
        </div>
